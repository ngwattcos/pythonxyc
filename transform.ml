...
| List l -> 
    Buffer.add_string buf "[";
    List.iter (fun exp -> transform_e exp buf; Buffer.add_string buf ", ") l;
    Buffer.add_string buf "]"
| Dict l ->
    Buffer.add_string buf "{";
    List.iter (fun ((e1, e2): exp * exp) -> transform_e e1 buf; Buffer.add_string buf ": "; transform_e e2 buf; Buffer.add_string buf ", ") l;
    Buffer.add_string buf "}"
| _ -> ()
...
and transform_var_access (v: var_access) (buf: Buffer.t) = 
match v with
