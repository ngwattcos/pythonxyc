...
let rec transform_p (prog: Ast.program) (buf: Buffer.t) =
transform_coms prog buf;
buf

and transform_coms (prog: Ast.program) (buf: Buffer.t) = match prog with
| [] -> ()
| c::[] ->
...
and transform_c (c: Ast.com) (buf: Buffer.t) = match c with
...
and transform_update_op (op: Ast.update_op) (buf: Buffer.t) = match op with
| Equals ->
    Buffer.add_string buf " = "
| PlusEquals ->
    Buffer.add_string buf " += "
...
| Paren (e) ->
    ...
    transform_e e buf;
    Buffer.add_string buf ")"
| Expon (a1, a2) -> failwith "unimplemented expon"
| Times (a1, a2) -> failwith "unimplemented times"
| Div (a1, a2) -> failwith "unimplemented divide"
| Plus (a1, a2) -> failwith "unimplemented plus"
| Minus (a1, a2) -> failwith "unimplemented minus"
| Mod (a1, a2) -> failwith "unimplemented mod"


and transform_func_call (fc: Ast.func_call) (buf: Buffer.t) = match fc with
...
and transform_params (lst: Ast.var list) (buf: Buffer.t) = match lst with
| [] -> ()
| param::[] ->
...
let transform (prog: Ast.program) =
